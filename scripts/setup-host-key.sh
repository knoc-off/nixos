...
echo ""
echo "Next steps:"
echo "  1. Build the image:    nix build .#images.${HOSTNAME}"
echo "  2. Decompress:         cp result/sd-image/*.img.zst ./${HOSTNAME}.img.zst && zstd -d ./${HOSTNAME}.img.zst"
echo "  3. Inject the key:     sudo ./scripts/inject-host-key.sh ./${HOSTNAME}.img ${KEY_PATH}"
echo "  4. Flash to SD card:   sudo dd if=./${HOSTNAME}.img of=/dev/sdX bs=4M status=progress"
