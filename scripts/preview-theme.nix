...
let
  ...
  bg =
    ...
    let
      rgb = hexToRgb h;
    in
    "${esc}[48;2;${toString rgb.r};${toString rgb.g};${toString rgb.b}m";
  fg =
    h:
    let
      rgb = hexToRgb h;
    in
    "${esc}[38;2;${toString rgb.r};${toString rgb.g};${toString rgb.b}m";
  rst = "${esc}[0m";
  bold = "${esc}[1m";
  ...
  cr =
    bgHex: hex:
    let
      v = toString (c.contrastRatio hex bgHex);
    in
    builtins.substring 0 4 v;
  accent =
    bgHex: label: hex:
    "${swatch label hex}  cr=${cr bgHex hex}";
  ...
  renderTheme =
    ...
    builtins.concatStringsSep "\n" [
      ...
      (builtins.concatStringsSep "" (
        lib.imap0 (i: hex: "${bg hex}  ${toString i} ${rst}") t.workspaceColors
      ))
      ""
    ];
