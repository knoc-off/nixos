...
mod tests {
    ...
    /// Render section 0 through the shared block path with an empty
    /// registry. These cloze cases contain only prose, so no external
    /// dispatch is involved -- this just exercises the inline HTML the
    /// parser baked (`{{c1::...}}`, `<strong>`, `<em>`).
    fn front_html(note: &Note) -> String {
        crate::render::Registry::new()
            .render_blocks(note.section(0), &PathBuf::new(), &PathBuf::new())
            .html
    }
