...
rec {
  ...
  testBasicArithmeticHelpers = {
    # Test floor
    testFloorPositive = assert math.floor 3.7 == 3;
    testFloorNegative = assert math.floor (-2.3) == -3;
    # Test ceil
  ...
  testMinMax = {
    # Test min
    testMinInt = assert math.min 5 3 == 3;
    testMinFloat = assert math.min 5.5 3.2 == 3.2;
    testMinMixed = assert math.min 5 3.2 == 3.2;
    # Test max
  ...
  testClamp = {
    # Test clamp
    testClampInside = assert math.clamp 5 0 10 == 5;
    testClampBelow = assert math.clamp (-2) 0 10 == 0;
  ...
  testBasicArithmeticOps = {
    ...
    # testSub = assert math.sub [10 3 2] == 5; # This seems incorrect based on definition
    # Test sum
    testSum = assert math.sum [1 2 3 4] == 10;
  ...
  testAbs = {
    # Test abs (int)
    testAbsPositiveInt = assert math.abs 5 == 5;
    testAbsNegativeInt = assert math.abs (-5) == 5;
    testAbsZeroInt = assert math.abs 0 == 0;
    # Test fabs (float)
  ...
  testArange = {
    ...
    testArange1 = assert math.arange 0 5 1 == [ 0 1 2 3 4 ];
    testArange2 = assert math.arange 1.0 3.0 0.5 == [ 1.0 1.5 2.0 2.5 ];
  ...
  testHasFraction = {
    # Test hasFraction
    testHasFractionTrue = assert math.hasFraction 12.34 == true;
  ...
  testDivMod = {
    ...
    testDivPositive = assert math.div 7 3 == 2;
    testDivNegativeNum = assert math.div (-7) 3 == -3; # Floor behavior
    testDivNegativeDen = assert math.div 7 (-3) == -3; # Floor behavior
    testDivBothNegative = assert math.div (-7) (-3) == 2;
    testDivExact = assert math.div 6 3 == 2;
    # Test mod
    testModPositive = assert math.mod 7 3 == 1;
    testModNegativeNum = assert math.mod (-7) 3 == 2; # Result has same sign as divisor
    testModNegativeDen = assert math.mod 7 (-3) == -2; # Result has same sign as divisor
    testModBothNegative = assert math.mod (-7) (-3) == -1;
    testModZero = assert math.mod 6 3 == 0;
  ...
  testPow = {
    ...
    testPowIntPositive = assert math.pow 2 3 == 8;
    testPowIntZero = assert math.pow 5 0 == 1;
    testPowIntNegativeBase = assert math.pow (-2) 3 == -8;
    testPowIntNegativeBaseEven = assert math.pow (-2) 4 == 16;
    # Test powFloat
  ...
  #   let f = x: { value = x*x - 2.0; deriv1 = 2.0*x; deriv2 = 2.0; };
  #   in assertWithinTolerance "halley (x^2-2)" (math.sqrt 2.0) (math.halley f 1.0) epsilon;
  # };

  testHexToDec = {
    # Test hexToDec
    testHexToDec0 = assert math.hexToDec "0" == 0;
    testHexToDecF = assert math.hexToDec "F" == 15;
    testHexToDec10 = assert math.hexToDec "10" == 16;
  ...
  testCoordinateSystems = {
    ...
    testCartToPolar1 = let p = equations.cartesianToPolar 1.0 1.0; in assertWithinTolerance "cartToPolar.r (1,1)" (math.sqrt 2.0) p.r epsilon && assertWithinTolerance "cartToPolar.theta (1,1)" (math.pi / 4.0) p.theta epsilon;
    testCartToPolar2 = let p = equations.cartesianToPolar (-1.0) 0.0; in assertWithinTolerance "cartToPolar.r (-1,0)" 1.0 p.r epsilon && assertWithinTolerance "cartToPolar.theta (-1,0)" math.pi p.theta epsilon;
  ...
  testAlgebraic = {
    ...
    testSolveQuadratic1 = let roots = equations.solveQuadratic 1.0 0.0 (-1.0); in assert (lib.lists.elem 1.0 roots && lib.lists.elem (-1.0) roots && builtins.length roots == 2);
  ...
  testVectorMath = {
    ...
    testNormalizeVector2D = let v = equations.normalizeVector2D 3.0 4.0; in assertWithinTolerance "normalizeVector2D.x" 0.6 v.x epsilon && assertWithinTolerance "normalizeVector2D.y" 0.8 v.y epsilon;
    # Test vectorAdd2D
    testVectorAdd2D = let v = equations.vectorAdd2D 1.0 2.0 3.0 4.0; in assertWithinTolerance "vectorAdd2D.x" 4.0 v.x epsilon && assertWithinTolerance "vectorAdd2D.y" 6.0 v.y epsilon;
