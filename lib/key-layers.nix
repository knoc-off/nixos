...
let
  ...
  compileKeyExpr =
    ...
    if action ? raw then
      ...
      in
      if action ? mod then "${modPrefix.${action.mod}}-${targetKey}" else targetKey;
  ...
  compileCapsAction =
    keyName: action:
    if action ? raw || action ? cmd then
      compileKeyExpr keyName action
    else
      let
        targetKey = action.key or keyName;
        keys = if action ? mod then "${modKey.${action.mod}} ${targetKey}" else targetKey;
      in
      "(unmod (rmet) ${keys})";
  ...
  normalizeCapsAction =
    keyName: action: if builtins.isString action then { mod = action; } else action;

  # In binds, string shorthand = target key name: "down" -> { key = "down"; }
  normalizeBindAction =
    keyName: action: if builtins.isString action then { key = action; } else action;

  # Fork compilation
  # Canonical name for a set of held modifiers (sorted alphabetically).
  modComboName =
    mods:
    if mods == [ ] then "default" else lib.concatStringsSep "_" (builtins.sort (a: b: a < b) mods);
  ...
  compileFork =
    ...
    let
        ...
        let
          tryMatch =
            ...
            if forkDef ? ${name} then
              ...
              in
              if lowest == null then null else tryMatch (builtins.filter (m: m != lowest) mods);
          actionValue = tryMatch heldMods;
        in
        if actionValue == null then
          keyName
        else
          compileKeyExpr keyName (normalizeBindAction keyName actionValue);
      ...
      buildTree =
        heldMods: remainingMods:
        if remainingMods == [ ] then
          resolveAction heldMods
        else
          let
          ...
          in
          if withoutMod == withMod then
            withoutMod
          else
            "(fork ${withoutMod} ${withMod} ${modForkKeys.${mod}})";
    in
  ...
  compileBind =
    keyName: value:
    if isFork value then
      compileFork keyName value
    else
      compileKeyExpr keyName (normalizeBindAction keyName value);
  ...
  normalizeCapsBinds =
    capsbinds:
    let
      bulkEntries = lib.foldl' (
        acc: mod:
        if capsbinds ? ${mod} then
          acc
          // lib.genAttrs capsbinds.${mod} (_: {
            inherit mod;
          })
        else
          acc
  ...
  presets = {
    ...
    navKeys = {
      ...
      d = {
        raw = "(multi (unmod (rmet) nop0) (mwheel-accel-down 50 150 1.05 0.80))";
      };
      u = {
        raw = "(multi (unmod (rmet) nop0) (mwheel-accel-up 50 150 1.05 0.80))";
      };
    };
    ...
    docNavG = {
      raw = "(tap-dance 200 ((unmod (rmet) lctl end) (unmod (rmet) lctl home)))";
    };
  };
  ...
  mkKeyLayers =
    ...
    let
      ...
      normalizedCaps = lib.mapAttrs (_: layer: normalizeCapsBinds (layer.capsbinds or { })) layers;
      ...
      allCapsAliases =
        ...
        lib.foldl' (
          ...
          in
          if acc ? ${name} then acc else acc // { ${name} = pair; }
        ) { } pairs;
      ...
      getCapsAliasName =
        ...
        let
          compiled = compileCapsAction keyName action;
        in
        "caps-${keyName}-${builtins.hashString "md5" compiled}";

      capsAliasLines = lib.mapAttrsToList (name: pair: "${name} ${pair.compiled}") allCapsAliases;
      ...
      getBindAliasName =
        ...
        let
          compiled = compileBind keyName value;
        in
        "remap-${keyName}-${builtins.hashString "md5" compiled}";

      bindAliasLines = lib.mapAttrsToList (name: pair: "${name} ${pair.compiled}") allBindAliases;
      ...
      mkCapsLayermap =
        ...
        lib.concatStringsSep "  " (
          lib.mapAttrsToList (keyName: action: "${keyName} @${getCapsAliasName keyName action}") keyMap
        );

      mkBindEntries =
        layer:
        lib.concatStringsSep "\n      " (
          lib.mapAttrsToList (keyName: value: "${keyName} @${getBindAliasName keyName value}") (
            layer.binds or { }
          )
        );
    in
