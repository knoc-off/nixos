...
let
  ...
  srgb_transfer_function =
    a: if a <= 0.0031308 then 12.92 * a else 1.055 * (math.powFloat a (1.0 / 2.4)) - 0.055;

  srgb_transfer_function_inv =
    a: if a <= 0.04045 then a / 12.92 else math.powFloat ((a + 0.055) / 1.055) 2.4;
  ...
  compute_max_saturation =
    ...
    let
      ...
      coeffs =
        if (-1.88170328 * a - 0.80936493 * b > 1.0) then
          {
            # Red component clips
            k0 = 1.19086277;
            k1 = 1.76576728;
  ...
  find_cusp =
    ...
    let
      ...
      rgb_at_max = oklab_to_linear_srgb {
        L = 1.0;
        a = S_cusp * a;
        b = S_cusp * b;
      };
  ...
  find_gamut_intersection_with_cusp =
    a: b: L1: C1: L0: cusp:
    let
      t_initial =
  ...
  toe =
    ...
    in
    0.5 * (k_3 * x - k_1 + math.sqrt (math.max 0.0 discriminant)); # Ensure sqrt input >= 0
  ...
  toe_inv =
    ...
    let
      ...
      k_3 = (1.0 + k_1) / (1.0 + k_2);
    in
    (x * x + k_1 * x) / (k_3 * (x + k_2) + epsilon); # Add epsilon for safety
          ...
          / (
            ...
            +
              ...
              * (
                0.40370612
                + 0.90148123 * b_
                + a_ * (-0.27087943 + 0.61223990 * b_ + a_ * (0.00299215 - 0.45399568 * b_ - 0.14661872 * a_))
              )
          );
    in
    {
...
rec {
      ...
      let
        ...
        C =
          if s < mid then
            ...
            in
            k_0 + t * k_1 / (1.0 - k_2 * t + epsilon);

        rgb_linear = oklab_to_linear_srgb {
          L = L;
          a = C * a_;
          b = C * b_;
        };
    ...
    let
      ...
      h =
        if C_cand < epsilon then
          ...
          in
          if h_norm < 0.0 then h_norm + 1.0 else h_norm; # Range [0, 1]
      s =
        if C_cand < epsilon then
          0.0
        else
          let
            ...
            s_calc =
              if C < C_mid then
                ...
                mid + (1.0 - mid) * t;
          in
          s_calc; # End of saturation calculation for non-grey colors
  ...
  okhsv_to_srgb =
    ...
    let
      ...
      rgb_scale = oklab_to_linear_srgb {
        L = L_vt;
        a = a_ * C_vt;
        b = b_ * C_vt;
      };
